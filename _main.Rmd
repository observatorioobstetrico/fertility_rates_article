---
title: 'Supplementary Material: Additional context to all results presented'
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, fig.align = "center", fig.width = 8, fig.height = 6, fig.pos='H')
options(scipen = 999)
```
...
<!--chapter:end:supplementary_material.Rmd-->

---
title: 'Supplementary Material: Additional analysis using correction of birth numbers based on under-registration estimates'
author: "Rafael Sant'Ana Herzog"
date: "`r Sys.Date()`"
bibliography: references.bib
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, fig.align = "center", fig.width = 8, fig.height = 6, fig.pos='H')
options(scipen = 999)
options(bookdown.post.prod = NULL)
```
...
## Municipalities with no births

All the years in the considered period included municipalities that had no births from mothers in each age group. 
...
In addition, 227 municipalities had no births from mothers aged 10 to 14 in the entire period. 
...
In an attempt to take into account factors such as under-registration of the number of live births in small municipalities, we corrected the numbers of live births using the SINASC under-registration estimates provided by the [IBGE](https://www.ibge.gov.br/estatisticas/sociais/populacao/26176-estimativa-do-sub-registro.html). Despite a small decrease in the number of municipalities that did not report births in each age group, as we can see from the output below, we decided not to use this approach because: 1) under-registration estimates are only available from 2015 onwards, making it impossible to correct the temporal analyses in this article, which consider the entire period from 2012 to 2021; 2) not all municipalities contained under-registration estimates; and 3) even for the analyses in which we could make this correction for the entire period considered (i.e the cluster analyses and modeling), new fitting attempts revealed that the correction had minimal influence on the results - for the modeling, for example, the differences in the estimated coefficients were in the third decimal place. Therefore, understanding that the macro scenario was not improved after the correction, we decided to calculate the fertility rates in a raw manner, without using any additional techniques in the process.
...
```{r}
# Checking how many municipalities had 0 births from mothers in each age group per year after the correction
kbl(
...
<!-- The municipalities that would be most affected by under-registration corrections for each age group can be seen in the interactive tables below (ordered, by default, by the percentage increase in the number of births). -->

<!-- **- For the 10 to 14 aged group:** -->

<!-- ```{r} -->
<!-- df_indicadores_corrigido |> -->
<!--   mutate( -->
<!--     births_added_10_to_14 = nvm_10_a_14_corrigido - nvm_10_a_14, -->
<!--     percentage_increase = round(nvm_10_a_14_corrigido / nvm_10_a_14 * 100 - 100, 1) -->
<!--   ) |> -->
...
<!--     State = uf, -->
<!--     Ano = ano, -->
<!--     `Original number of births (10-14)` = nvm_10_a_14, -->
<!--     `Corrected number of births (10-14)` = nvm_10_a_14_corrigido, -->
<!--     `Total births added (10-14)` = births_added_10_to_14, -->
<!--     `% increase` = percentage_increase -->
<!--   ) |> -->
<!--   reactable(filterable = TRUE) -->
<!-- ``` -->

<!-- **- For the 15 to 19 aged group:** -->

<!-- ```{r} -->
<!-- df_indicadores_corrigido |> -->
<!--   mutate( -->
<!--     births_added_15_to_19 = nvm_15_a_19_corrigido - nvm_15_a_19, -->
<!--     percentage_increase = round(nvm_15_a_19_corrigido / nvm_15_a_19 * 100 - 100, 1) -->
<!--   ) |> -->
<!--   arrange(desc(percentage_increase)) |> -->
<!--   select( -->
<!--     Codmunres = codmunres, -->
...
<!--     State = uf, -->
<!--     Ano = ano, -->
<!--     `Original number of births (15-19)` = nvm_15_a_19, -->
<!--     `Corrected number of births (15-19)` = nvm_15_a_19_corrigido, -->
<!--     `Total births added (15-19)` = births_added_15_to_19, -->
<!--     `% increase` = percentage_increase -->
<!--   ) |> -->
<!--   reactable(filterable = TRUE) -->
<!-- ``` -->

<!-- **- For the 10 to 19 aged group:** -->

<!-- ```{r} -->
<!-- df_indicadores_corrigido |> -->
<!--   mutate( -->
<!--     births_added_10_to_19 = nvm_10_a_19_corrigido - nvm_10_a_19, -->
<!--     percentage_increase = round(nvm_10_a_19_corrigido / nvm_10_a_19 * 100 - 100, 1) -->
<!--   ) |> -->
<!--   arrange(desc(percentage_increase)) |> -->
<!--   select( -->
<!--     Codmunres = codmunres, -->
...
<!--     Ano = ano, -->
<!--     State = uf, -->
<!--     `Original number of births (10-19)` = nvm_10_a_19, -->
<!--     `Corrected number of births (10-19)` = nvm_10_a_19_corrigido, -->
<!--     `Total births added (10-19)` = births_added_10_to_19, -->
<!--     `% increase` = percentage_increase -->
<!--   ) |> -->
<!--   reactable(filterable = TRUE) -->
<!-- ``` -->
...
avaliar_cluster_indices <- function(tipo_taxa, anos, kmeans_n, pam_n, ward_n) {
  ...
  kmeans_index <- data.frame(
    metodo = unlist(lapply(kmeans_n, function(i) paste0("kmeans", i))),
  ...
  pam_index <- data.frame(
    metodo = unlist(lapply(pam_n, function(i) paste0("pam", i))),
  ...
  ward_index <- data.frame(
    metodo = unlist(lapply(ward_n, function(i) paste0("ward", i))),
...
```{r echo=FALSE, fig.cap="K-Means knee-plot for the 10 to 14 age group and 2018-2019 period"}
...
```{r echo=FALSE, fig.cap="K-Means knee-plot for the 10 to 14 age group and 2020-2021 period"}
...
```{r echo=FALSE, fig.cap = "K-Medians knee-plot for the 10 to 14 age group and 2018-2019 period"}
...
```{r echo=FALSE, fig.cap = "K-Medians knee-plot for the 10 to 14 age group and 2020-2021 period"}
...
```{r fig.cap="Dendrogram for Ward's method for the 10 to 14 age group and 2018-2019 period"}
# Ploting the drodrogram for the 2018-2019 period
set.seed(1504)
d1_ward_18_19 <- hclust(dados1_18_19_dist, method = "ward.D2")
plot(as.dendrogram(d1_ward_18_19))
...
```{r fig.cap="Dendrogram for Ward's method for the 10 to 14 age group and 2020-2021 period"}
# Ploting the drodrogram for the 2020-2021 period
set.seed(1504)
d1_ward_20_21 <- hclust(dados1_20_21_dist, method = "ward.D2")
plot(as.dendrogram(d1_ward_20_21))
...
```{r fig.cap = "Dendrogram for the single linkage method for the 10 to 14 age group and 2018-2019 period"}
# Ploting the dendrogram for the 2018-2019 period
set.seed(1504)
d1_single_18_19 <- hclust(dados1_18_19_dist, method = "single")
plot(as.dendrogram(d1_single_18_19))
...
```{r fig.cap="Dendrogram for the single linkage method for the 10 to 14 age group and 2020-2021 period"}
# Ploting the dendrogram for the 2020-2021 period
set.seed(1504)
d1_single_20_21 <- hclust(dados1_20_21_dist, method = "single")
plot(as.dendrogram(d1_single_20_21))
...
```{r fig.cap="Dendrogram for the complete linkage method for the 10 to 14 age group and 2018-2019 period"}
...
```{r fig.cap="Dendrogram for the complete linkage method for the 10 to 14 age group and 2020-2021 period"}
...
```{r fig.cap="Dendrogram for the average linkage method for the 10 to 14 age group and 2018-2019 period"}
...
```{r fig.cap="Dendrogram for the average linkage method for the 10 to 14 age group and 2020-2021 period"}
...
* silh_index: the higher the better (pam4, followed by ward3, kmeans3 and kmeans4);
* ch_index_cent: the higher the better (kmeans4, followed by kmeans3 and ward3);
* ch_index_med: the higher the better (kmeans4, followed by kmeans3 and ward3).

**Choice:** despite the better perfomance of K-Means with K = 4, one of its clusters contained only a few municipalities (46).

```{r}
d1_kmeans4_18_19$size
```

Thus, the chosen method is the **K-Means with K = 3**, since it had the best average performance after K-Means with K = 4.
...
* db_index_cent: the lower the better (kmeans4, followed by kmeans3);
* db_index_med: the lower the better (kmeans4, followed by kmeans3);
* dunn_index: the higher the better (kmeans4, followed by kmeans3);
* silh_index: the higher the better (ward2, followed by kmeans3);
...
**Choice:** again, despite the better perfomance of K-means with K = 4, one of its clusters contained only a few municipalities (93).

```{r}
d1_kmeans4_20_21$size
```

Thus, the chosen method is the **K-Means with K = 3**, since it had the best overall performance after K-means with K = 4.
...
```{r echo=FALSE, fig.cap="K-Means knee-plot for the 15 to 19 age group and 2018-2019 period"}
...
```{r echo=FALSE, fig.cap="K-Means knee-plot for the 15 to 19 age group and 2020-2021 period"}
...
```{r echo=FALSE, fig.cap="K-Medians knee-plot for the 15 to 19 age group and 2018-2019 period"}
...
```{r echo=FALSE, fig.cap="K-Medians knee-plot for the 15 to 19 age group and 2020-2021 period"}
...
```{r fig.cap="Dendrogram for Ward's method for the 15 to 19 age group and 2018-2019 period"}
# Ploting the drodrogram for Ward's method for the 2018-2019 period
set.seed(1504)
d2_ward_18_19 <- hclust(dados2_18_19_dist, method = "ward.D2")
plot(as.dendrogram(d2_ward_18_19))
...
```{r fig.cap="Dendrogram for Ward's method for the 15 to 19 age group and 2020-2021 period"}
# Ploting the drodrogram for Ward's method for the 2020-2021 period
set.seed(1504)
d2_ward_20_21 <- hclust(dados2_20_21_dist, method = "ward.D2")
plot(as.dendrogram(d2_ward_20_21))
...
```{r fig.cap="Dendrogram for the single linkage method for the 15 to 19 age group and 2018-2019 period"}
# Ploting the drodrogram for the single linkage method for the 2018-2019 period
set.seed(1504)
d2_single_18_19 <- hclust(dados2_18_19_dist, method = "single")
plot(as.dendrogram(d2_single_18_19))
...
```{r fig.cap="Dendrogram for the single linkage method for the 15 to 19 age group and 2020-2021 period"}
# Ploting the drodrogram for the single linkage method for the 2020-2021 period
set.seed(1504)
d2_single_20_21 <- hclust(dados2_20_21_dist, method = "single")
plot(as.dendrogram(d2_single_20_21))
...
```{r fig.cap="Dendrogram for the complete linkage method for the 15 to 19 age group and 2018-2019 period"}
# Ploting the drodrogram for the complete linkage method for the 2018-2019 period
set.seed(1504)
d2_complete_18_19 <- hclust(dados2_18_19_dist, method = "complete")
plot(as.dendrogram(d2_complete_18_19))
...
```{r fig.cap="Dendrogram for the complete linkage method for the 15 to 19 age group and 2020-2021 period"}
# Ploting the drodrogram for the complete linkage method for the 2020-2021 period
set.seed(1504)
d2_complete_20_21 <- hclust(dados2_20_21_dist, method = "complete")
plot(as.dendrogram(d2_complete_20_21))
...
```{r fig.cap="Dendrogram for the average linkage method for the 15 to 19 age group and 2018-2019 period"}
# Ploting the drodrogram for the average linkage method for the 2018-2019 period
set.seed(1504)
d2_average_18_19 <- hclust(dados2_18_19_dist, method = "average")
plot(as.dendrogram(d2_average_18_19))
...
```{r fig.cap="Dendrogram for the average linkage method for the 15 to 19 age group and 2020-2021 period"}
# Ploting the drodrogram for the average linkage method for the 2020-2021 period
set.seed(1504)
d2_average_20_21 <- hclust(dados2_20_21_dist, method = "average")
plot(as.dendrogram(d2_average_20_21))
...
* db_index_cent: the lower the better (ward3, followed by kmeans3)
* db_index_med: the lower the better (ward3, followed by kmeans3)
* dunn_index: the higher the better (kmeans4, followed by kmeans3)
* silh_index: the higher the better (kmeans3, followed by kmeans4)
* ch_index_cent: the higher the better (kmeans4, followed by kmeans3)
* ch_index_med: the higher the better (kmeans4, followed by kmeans3)

**Choice:** K-Means with K = 4 (since, this time, the number of observations in all of its clusters were adequate).
...
* db_index_med: the lower the better (kmeans4, followed by kmeans3);
* dunn_index: the higher the better (kmeans4, followed by kmeans3);
* silh_index: the higher the better (ward2, followed by kmeans4);
* ch_index_cent: the higher the better (kmeans4, followed by kmeans3);
* ch_index_med: the higher the better (kmeans4, followed by kmeans3).
...
```{r echo=FALSE, fig.cap="Distribution of groups of municipalities formed by municipalities with similar fertility rate of women aged 10 to 14 years in the pre-pandemic period (on the left) and in the pandemic period (on the right).", out.width='100%'}
...
```{r echo=FALSE, fig.cap="Distribution of groups of municipalities formed by municipalities with similar fertility rate of women aged 15 to 19 years in the pre-pandemic period (on the left) and in the pandemic period (on the right).", fig.width=12, out.width='100%'}
...
cria_tabelas_testes <- function(
  ...
  for (variavel in variaveis) {
    ...
    if (length(unique(resultados_dunn$group1)) == 2) {
      df_teste <- data.frame(
    ...
    } else if (length(unique(resultados_dunn$group1)) == 3) {
      df_teste <- data.frame(
        grupo1 = monta_linhas(1),
        grupo2 = c(rep("", 1), monta_linhas(2)),
...
```{r wp1, fig.cap="Worm plot of the selected model for the skew t type 4 distribution."}
...
```{r wp2, fig.cap="Worm plot of the selected model for the ZAGA distribution."}
...
```{r interpretation, fig.cap="Predicted values of the fertility rate of women under 20 years of age according to HDI-M values, Primary Care Population coverage and information on the year of the pandemic. Brazil, 2018-2021."}
...
<!--chapter:end:supplementary_material2.Rmd-->
